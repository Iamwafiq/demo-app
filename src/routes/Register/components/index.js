  ...
  const registerUser = () => {
    ...
    if(!firstName.length && !error){
      ...
      if(!reg3.test(firstName) && !error){
        error = true;
        new Toast("First Name should have alphabets only", Toast.TYPE_ERROR, time);
      }
    }
    ...
    if(!lastName.length && !error){
      ...
      if(!reg3.test(lastName) && !error){
        error = true
        new Toast("Last Name should have alphabets only", Toast.TYPE_ERROR, time);
      }
    }
    ...
    if(!address.length && !error){
      ...
      error=true
      new Toast("Address cannot be less than 20 characters", Toast.TYPE_ERROR, time);
    }
      ...
      <div className={loginPage}>
        ...
        <div className={loginText}>Fill in the Registration Form</div>
          <div className={fields}>
            <div className={eachLine}>
              <div className={imageBox}>
                <label for="upload-image">
                  <img alt="icon" className={image && image.length?"":addImage} width="100%" height="100%" src={image && image.length?image:plusIcon}/>
                </label>
              </div>             
              <input type="file" 
              key={new Date()} 
            ...
            <div className={eachLine}>
              <TextBox value={firstName} name="firstName" className={input} type="text" placeholder="Last Name" onChange={setFirstName}/>
              <TextBox value={lastName} name="lastName" className={input} type="text" placeholder="Last Name" onChange={setLastName}/>
            </div>
            <div className={eachLine}>
              <TextBox value={email} name="email" className={input} type="email" placeholder="Enter Email Id" onChange={setEmail}/>
              <TextBox value={phone} name="phone" className={input} type="text" placeholder="Enter Phone Number" onChange={setPhone}/>
            </div>
            <div className={eachLine}>
              <div className={inputSingle} style={{"border":"none","display":"flex"}}>
                <div className={radioText}>
                <div className={inboxText}>Select Gender:</div>
                </div>
                <div className={singleRadio}>
                  <input type="radio" onBlur={(e) => setGender(e.target.value)} id="male" name="gender" value="male"/>
                  <label for="male" style={{"margin-left":"0.2em"}}>Male</label>
                </div>
                <div className={singleRadio}>
                  <input type="radio" onBlur={(e) => setGender(e.target.value)} id="female" name="gender" value="female"/>
                  <label for="female" style={{"margin-left":"0.2em"}}>Female</label>
                </div>
              </div>
            </div>
            <div className={eachLine}>
              <TextBox value={address} name="address" inputType="text" className={inputSingleText} type="text" placeholder="Enter Address" onChange={setAddress}/>
            </div>
            <div className={eachLine}>
              <TextBox value={password} name="password" className={input} type="password" placeholder="Enter Password" onChange={setPassword}/>
              <TextBox value={cPassword} name="confirmPassword" className={input} type="password" placeholder="Confirm Password" onChange={setcPassword}/>
            </div>
